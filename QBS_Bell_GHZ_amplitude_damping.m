...
while(t(end)<=2^7)
    t=t+4;
    if t(end)<=2^7
    rho_out_ampdamp=collect(rho_out_ampdamp+rho_all_ampdamp(t,t));
    end
end
disp(rho_out_ampdamp);
    
% calculate the fidelity
f_out_ampdamp=collect(phi_in'*rho_out_ampdamp*phi_in);disp('f_out_ampdamp =');disp(f_out_ampdamp);