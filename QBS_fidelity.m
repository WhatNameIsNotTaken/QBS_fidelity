...
for i=1:size(p,2)
    E0(:,:,i)=sqrt(p(i))*[1 0;0 1];
    E1(:,:,i)=sqrt(1-p(i))*[1 0;0 -1];
...
rho_all_phasefilp=U*rho_source_phaseflip*U';
t=(1:4);
rho_out_phaseflip=rho_all_phasefilp(t,t);
while(t(end)<=2^7)
    t=t+4;
    if t(end)<=2^7
    rho_out_phaseflip=rho_out_phaseflip+rho_all_phasefilp(t,t);
    end
end
f_out_phasefilp(i)=phi_in'*rho_out_phaseflip*phi_in;
end
plot(p,f_out_phasefilp)
hold on;
...
rho_source_bitflip=simplify(e1*rho_source*e1'+e2*rho_source*e2'+e3*rho_source*e3'+e4*rho_source*e4'+e5*rho_source*e5'+e6*rho_source*e6'+...
    e7*rho_source*e7'+e8*rho_source*e8'+e9*rho_source*e9'+e10*rho_source*e10'+e11*rho_source*e11'+e12*rho_source*e12'+e13*rho_source*e13'+...
    e14*rho_source*e14'+e15*rho_source*e15'+e16*rho_source*e16');
rho_all_bitfilp=U*rho_source_bitflip*U';
t=(1:4);
rho_out_bitflip=rho_all_bitfilp(t,t);
while(t(end)<=2^7)
    t=t+4;
    if t(end)<=2^7
    rho_out_bitflip=rho_out_bitflip+rho_all_bitfilp(t,t);
    end
end
f_out_bitfilp(i)=phi_in'*rho_out_bitflip*phi_in;
end
plot(p,f_out_bitfilp)
legend('phase flipping noise','depolarzing noise','amplitude damping noise','bit flipping noise')
hold on;